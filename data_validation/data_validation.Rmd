...
```{r annual-precip, include=FALSE}
#calculate total annual precip means and SDs
# Observed
xa_tap <- 
  xa_raw %>% 
  mutate(year = str_extract(yearmonth, "\\d{4}")) %>% 
  group_by(year) %>% 
  summarise(tap = sum(pr)) %>% 
  pull(tap)
...
cmip_tap <- 
  map(df_list, 
      ~ .x %>% 
        ...
        group_by(year) %>% 
        summarize(tap = sum(pr) %>% as.numeric()) %>% 
        pull(tap)
  )
# ± 2SD
mean(xa_tap) - sd(xa_tap)*2
mean(xa_tap) + sd(xa_tap)*2
...
# Quantiles
quantile(xa_tap, c(0.10, 0.5, 0.9))

# ± 95%CI
library(boot)
boot(xa_tap, function(x, i) c(mean(x[i])), R = 999) %>% boot.ci(conf = .95)

tap_df <-
  map_df(cmip_tap,
       ~ tibble(
         pr_mean_annual = mean(.x),
         diff = mean(.x) - mean(xa_tap)
       ),
       .id = "source_id") %>%
  mutate(within20 = between(pr_mean_annual, mean(xa_tap) * 0.8, mean(xa_tap) * 1.2)) %>%
  add_row(
    source_id = "observed",
    pr_mean_annual = mean(xa_tap),
    diff = 0,
    within20 = NA,
    .before = 1
  ) %>% 
  mutate(across(c(pr_mean_annual, diff), round)) %>% 
    mutate(pr_mean_annual = glue::glue("{pr_mean_annual} ({diff})")) %>% 
    select(-diff)
```
...
table_formatted <- 
  ...
  # highlight significant p-values for goodness of fit test
  tab_style(
    style = list(
      cell_text(color = "red")
    ),
  ...
  # highlight mean annual precipitation not within 20% of observed
  tab_style(
    style = list(
      cell_text(color = "red")
    ),
    locations = cells_body(
      columns = c(pr_mean_annual),
      rows = !within20
    )
  ) %>% 
  cols_hide(within20) %>% 
  ...
  cols_label(
    source_id = "Source",
    pr_mean_annual = "Mean annual pr (∆obs) [mm/yr]",
    pr_cor = "pr",
    tas_cor = "tas",
    tasmin_cor = "tasmin",
    tasmax_cor = "tasmax",
  ...
  tab_footnote("Red indicates mean annual precipitation not within ± 20% of observed",
               cells_column_labels(pr_mean_annual)) %>% 
...
plot_all <- function(data) {
  p_list <- map(c("tas", "tasmin", "tasmax", "pr", "hfls", "hfss", "pet", "spei"),
      ~ggplot(data,
