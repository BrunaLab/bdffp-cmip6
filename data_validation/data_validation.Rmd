...
```{r include=FALSE}
xa <- read_csv(here("data", "xavier_aggregated.csv"))
xa <- 
  xa %>%
  mutate(date = paste(yearmonth, 1) %>% ymd(),
         month = month(date, label = TRUE)) %>% 
  group_by(month) %>% summarize(across(c(pr, starts_with("tas")), mean))
```

```{r correlation, include=FALSE}
#function to calculate correlation to observed.
corr_season <- function(x) {
  df <- x %>% 
  ...
  tibble(
    pr = cor(df$pr, xa$pr),
    tas = cor(df$tas, xa$tas),
    tasmin = cor(df$tasmin, xa$tasmin),
    tasmax = cor(df$tasmax, xa$tasmax),
...
Correlation to observed monthly means:

```{r echo=FALSE}
corr_season(df_list[[i]])
```
